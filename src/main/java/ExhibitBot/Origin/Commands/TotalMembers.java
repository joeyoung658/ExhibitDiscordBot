    ...
    public void onMessageReceived(MessageReceivedEvent e){
        ...
        if (message.equalsIgnoreCase("!membercount") && !(e.getAuthor().isBot())) {
            ...
            if (e.isFromType(ChannelType.TEXT)) {
                e.getTextChannel().sendMessage( e.getAuthor().getAsMention()+  " There are " + e.getGuild().getMembers().size() + " members on the jda server!").queue();
            } else {
                e.getTextChannel().sendMessage("This command will only work within a jda server!");
            }
